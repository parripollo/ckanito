...
``package_search`` with full text search, facets, ranges and sorting. Solr,
``pysolr`` and the ``solr_*`` config options are gone; anyone needing Solr
again can implement the contract in an extension. Run ``ckan db upgrade`` and ``ckan search-index rebuild`` after upgrading.
